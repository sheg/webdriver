...
public class EditPlaylistPage extends BasePage
{
  ...
  /**
   * Shows current Dynamic Playlist Count #TextField
   */
   ...
   */
  public void validateValuesForDynamicPlaylist(PlaylistType type, int size, boolean includeHidden, String title,
      String description, String category, String shortDescription)  throws Exception {

    validatePlaylistType(type);
    validatePlaylistSize(size);
    validateIncludeHiddenInDynamicPlaylists(includeHidden);
    validateTitle(title);
    validateLongDescription(description);
    validateCategory(category);
    validateShortDescription(description, shortDescription);
  }

  /**
   * Validates the created playlist has the expected values presented in the parameters:
   *
   * @param type {@link PlaylistType#MANUAL}
   * @param size corroborates the size of the playlist
   * @param content Title of the content added in the creation of the playlist
   * @param title {@link String}, the title of the created playlist
   * @param description {@link String}, the description of the created playlist
   * @param category Checks {@link ContentCategory} is expected
   * @param shortDescription if it's not copied from Description, check custom
   * @throws Exception 
   */
  public void validateValuesForManualPlaylist(PlaylistType type, String[] content,
      String title, String description, String category, String shortDescription) throws Exception {
  ...
  /**
   * @param content An array of String with the titles of the content that the newly 
   * created playlist should contain.
   */
  private void validateManualContent(String[] content) throws Exception {
    for (int i = 1; i <= content.length; i++) {
      String title = getDriver().findElement(By.xpath("//ul[@id='playlist-preview']/li[" + i + "]/span/a")).getText();
      if (!title.equals(content[i - 1])) {
        throw new Exception("Playlist Content Error. " +
            "Expected [" + content[i - 1] + "], but got [" + title + "]");
      }
    }
  }

  private void validateShortDescription(String description, String shortDescription) throws Exception {
  ...
  private void validateCategory(String category) throws Exception {
    ...
    String selected_option = select.getFirstSelectedOption().getText().trim();
    if(!selected_option.equals(category)) {
      throw new Exception("Category does not match. Expected [" + category + "]," +
          " but got [" + selected_option + "].");
    }
  }

  private void validateLongDescription(String description) throws Exception {
  ...
  private void validateIncludeHiddenInDynamicPlaylists(boolean includeHidden) throws Exception {
  ...
  private void validatePlaylistSize(int size) throws Exception {
  ...
  private void validatePlaylistType(PlaylistType type) throws Exception {
