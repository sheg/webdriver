 ...
 */
public class TestAMSLivePlayerSaveSettingsTwice extends TestCase {
   ...
   */
  @Test
  public void testChangeContentTypeSettingsToVideoSize240x205AutoOff () throws Exception {
    ...
    // Change Config for a second time after save
    height = m_config.getValue("height_2");
    width = m_config.getValue("width_2");
   
    type = m_config.getValue("contentType_2").equals("video") ? ContentType.VIDEO : ContentType.AUDIO;
    autoplay = m_config.getValue("autoplay_2").equals("off") ? Autoplay.OFF : Autoplay.ON;
    size_param = m_config.getValue("size_2");
    if (type == ContentType.AUDIO) {
      size = Size.AUDIO;
    } else {
      size = size_param.equals("400x325") ? Size.s400x325 :
