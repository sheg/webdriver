 ...
 */
public class TestAMSLivePlayerSetSettingsVideoSize240x205AutoOff extends TestCase {
  ...
  public void testChangeContentTypeSettingsToVideoSize240x205AutoOff () throws Exception {
    ...
    if (livePlayer.isCurrentTypeAudio()) {
      livePlayer.enableSizeEditing();
      livePlayer.changeSize(size, height, width);
      livePlayer.disableSizeEditing();
    } else {
      livePlayer.changeSize(size, height, width);
    }
